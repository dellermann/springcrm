    ...
    <g:each var="dunning" in="${dunningList}" status="i">
      <tr>
        <td class="col-type-id dunning-number">
          <g:link controller="dunning" action="show" id="${dunning.id}">
        ...
        <td class="col-type-date dunning-subjectdue-date-payment">
          <g:formatDate date="${dunning?.dueDatePayment}"
            formatName="default.format.date"/>
        </td>
        <td class="col-type-currency dunning-total">
          <g:formatCurrency number="${dunning?.total}" displayZero="true"
            external="true"/>
        </td>
        <td class="col-type-currency dunning-closing-balance balance-state
            balance-state-${dunning?.balanceColor}">
        ...
        <td class="col-actions">
          ...
          <g:button controller="dunning" action="edit" id="${dunning.id}"
            color="success" size="xs" icon="pencil-square-o"
            message="default.button.edit.label"/>
          </g:if>
          <g:else>
          <g:button controller="dunning" action="editPayment"
            id="${dunning.id}" color="success" size="xs"
