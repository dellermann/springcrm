<g:applyLayout name="${orientation == 'vertical' ? 'field-vertical' : 'field'}">
  ...
  <g:set var="toolbarItems" value="${toolbar?.split(/\s+/) ?: []}"/>
  <div class="textarea-container">
    <textarea name="${property}" id="${property}"
      class="form-control${toolbarItems.contains('none') ? ' no-toolbar' : ''}"
      rows="${rows ?: 5}"${required ? ' required="required"' : ''}
      data-toggle="popover" data-placement="bottom" data-container="body"
      data-trigger="manual"
      ${toolbarItems.contains('none') ? '' : 'aria-controls="${property}-toolbar" aria-live="polite" aria-relevant="text"'}
      >${value}</textarea>
    <g:unless test="${toolbarItems.contains('none')}">
