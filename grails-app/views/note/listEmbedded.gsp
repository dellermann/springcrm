<g:applyLayout name="listEmbedded" model="[list: noteList, total: noteCount]">
  <table class="table data-table">
    <thead>
      <tr>
        <g:sortableColumn property="number"
          title="${message(code: 'note.number.label', default: 'Number')}"/>
        <g:sortableColumn property="title"
          title="${message(code: 'note.title.label', default: 'Title')}"/>
        <g:ifModuleAllowed modules="CONTACT">
        <g:sortableColumn property="person.lastName"
          title="${message(code: 'note.person.label', default: 'Person')}"/>
        </g:ifModuleAllowed>
        <th></th>
      </tr>
    </thead>
      ...
      <g:each var="note" in="${noteList}" status="i">
        <tr>
          ...
          <td class="col-type-ref note-person">
            <g:link controller="person" action="show" id="${note.person?.id}">
              <g:fieldValue bean="${note}" field="person"/>
            </g:link>
          </td>
          </g:ifModuleAllowed>
          <td class="col-actions">
            <g:button controller="note" action="edit" id="${note.id}"
