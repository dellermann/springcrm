<g:applyLayout name="listEmbedded"
  model="[list: projectList, total: projectCount]">
      ...
      <tr>
        <td class="col-type-id project-number">
          <g:link controller="project" action="show" id="${project.id}">
            <g:fullNumber bean="${project}"/></g:link>
        </td>
        ...
        <td class="col-type-string project-status project-status
            project-status-${project.status.id}">
          <g:fieldValue bean="${project}" field="status"/>
        </td>
        <td class="col-actions">
          <g:button controller="project" action="edit" id="${project.id}"
