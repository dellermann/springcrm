...
<table class="content-table">
  <thead>
    <tr>
      ...
      <g:sortableColumn property="docDate" title="${message(code: 'purchaseInvoice.docDate.label', default: 'Date')}" params="${linkParams}" />
      <g:sortableColumn property="dueDate" title="${message(code: 'purchaseInvoice.dueDate.label', default: 'Due date')}" params="${linkParams}" />
      <g:sortableColumn property="stage" title="${message(code: 'purchaseInvoice.stage.label', default: 'Stage')}" params="${linkParams}" />
      <th></th>
    </tr>
  </thead>
  <tbody>
  <g:each in="${purchaseInvoiceInstanceList}" status="i" var="purchaseInvoiceInstance">
    <tr>
      ...
      <td>
        ...
        <g:link controller="purchaseInvoice" action="delete" id="${purchaseInvoiceInstance.id}" class="button small red" onclick="return confirm(springcrm.messages.deleteConfirmMsg);"><g:message code="default.button.delete.label" /></g:link>
      </td>
    </tr>
  </g:each>
  </tbody>
