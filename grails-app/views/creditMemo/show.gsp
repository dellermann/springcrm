<html>
  ...
  <body>
    <g:applyLayout name="show" model="[instance: creditMemo]">
      ...
      <section>
        ...
        <div class="column-group">
          <div class="column">
            <f:display bean="${creditMemo}" property="number"/>
            <f:display bean="${creditMemo}" property="subject"/>
            <f:display bean="${creditMemo}" property="organization"/>
            <f:display bean="${creditMemo}" property="person"/>
            <f:display bean="${creditMemo}" property="invoice"/>
            <f:display bean="${creditMemo}" property="dunning"/>
            <f:display bean="${creditMemo}" property="stage"/>
            <f:display bean="${creditMemo}" property="createUser"/>
          </div>
          <div class="column">
            <f:display bean="${creditMemo}" property="docDate"/>
            <f:display bean="${creditMemo}" property="shippingDate"/>
            <f:display bean="${creditMemo}" property="carrier"/>
            <f:display bean="${creditMemo}" property="paymentDate"/>
            <f:display bean="${creditMemo}" property="paymentAmount"/>
            <f:display bean="${creditMemo}" property="paymentMethod"/>
            <f:display bean="${creditMemo}" property="closingBalance"/>
          </div>
        </div>
      </section>
      <section class="column-group">
        <f:display bean="${creditMemo}" property="billingAddr"
          title="${message(code: 'invoicingTransaction.fieldset.billingAddr.label')}"/>
        <f:display bean="${creditMemo}" property="shippingAddr"
      ...
      <section>
        ...
        <div class="column-group">
          <div class="column">
            <f:display bean="${creditMemo}" property="headerText"/>
          </div>
        </div>
      </section>
      <section>
      ...
      <section>
        ...
        <div class="column-group">
          <div class="column">
            <f:display bean="${creditMemo}" property="footerText"/>
            <f:display bean="${creditMemo}" property="termsAndConditions"/>
          </div>
        </div>
      </section>
      <g:if test="${creditMemo?.notes}">
      <section>
        <header>
          <h3><g:message
        ...
        <div class="column-group">
          <div class="column">
            <f:display bean="${creditMemo}" property="notes"/>
          </div>
        </div>
      </section>
      </g:if>
    ...
    <content tag="toolbar">
      ...
      <g:button action="edit" id="${creditMemo?.id}" color="success"
      ...
      <g:button action="copy" id="${creditMemo?.id}" color="primary"
        class="hidden-xs" icon="copy" message="default.button.copy.label"/>
      <g:if test="${session.credential.admin ||
          creditMemo.stage.id < 2502}">
      <g:button action="delete" id="${creditMemo?.id}" color="danger"
      ...
      <ul id="show-toolbar-menu" class="dropdown-menu" role="menu"
        ...
        <li role="menuitem">
          <g:link action="copy" id="${creditMemo?.id}">
        ...
        <g:if test="${session.credential.admin ||
            creditMemo.stage.id < 2502}">
        <li role="menuitem">
          <g:link action="delete" id="${creditMemo?.id}"
