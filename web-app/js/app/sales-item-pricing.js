...
(function() {
  var $, SalesItemPricing;

  $ = jQuery;

  SalesItemPricing = {
    INPUT_FIELD_NAMES: ["id", "quantity", "unit", "name", "type", "relToPos", "unitPercent", "unitPrice"],
    _$finderRow: null,
    _form: null,
    ...
    _create: function() {
      var $form, $trs, el, form, initialPricingEnabled, itemReferences, opts, pricingEnabled,
        _this = this;
      $ = jQuery;
      el = this.element;
      opts = this.options;
      ...
      $trs.each(function(index, elem) {
        ...
        ref = (_this._getRowType($tr) === "relativeToPos" ? _this._getFieldVal($tr, "relative-to-pos") : -1);
        return itemReferences.push(ref);
      });
    ...
    _getReferrers: function(idx) {
      var i, ref, _i, _len, _ref, _results;
      _ref = this._itemReferences;
      _results = [];
    ...
    _moveItem: function($tr, dir) {
      ...
      if ((dir < 0) && (pos > 0)) {
        $destTr = $tr.prev();
        $destTr.before($tr);
      } else if (pos < this._getRows().length - 1) {
        $destTr = $tr.next();
        $destTr.after($tr);
      }
      if ($destTr) {
    ...
    _onClickReferenceItem: function($target) {
      ...
      if (!$tr.is($finderRow)) {
        ...
        $finderRow.find("> .relative-to-pos > span").find("> strong").text(String(v + 1)).end().find("> input").val(v);
    ...
    _removeItemReference: function(idx) {
      var i, ref, refs, _i, _len, _results;
    ...
    _startFinderMode: function(item) {
      ...
      this._$finderRow = $tr;
      $tr.addClass("non-selectable").siblings().addClass("selectable");
    ...
    _stopFinderMode: function() {
      ...
      return $(window.document).unbind("keydown");
    },
    _swapInputItemPos: function($tr, $destTr) {
      var destIndex, fieldNames, form, index, swap,
        _this = this;
      form = this._form;
      ...
      swap(this._getInputName(index), this._getInputName(destIndex, "", "-dest"));
      swap(this._getInputName(destIndex), this._getInputName(index));
      return swap(this._getInputName(destIndex, "", "-dest"), this._getInputName(destIndex));
    },
    ...
    _swapItemReferences: function(item, destItem) {
      var destIdx, destRefs, idx, itemRefs, ref, refs, _i, _j, _k, _l, _len, _len1, _len2, _len3, _results;
      idx = this._getIndex(item);
      destIdx = this._getIndex(destItem);
      refs = this._getReferrers(idx);
      for (_i = 0, _len = refs.length; _i < _len; _i++) {
        ref = refs[_i];
        this._setItemReference(ref, destIdx);
      }
      destRefs = this._getReferrers(destIdx);
      for (_j = 0, _len1 = destRefs.length; _j < _len1; _j++) {
        ref = destRefs[_j];
        this._setItemReference(ref, idx);
      }
      itemRefs = this._itemReferences;
      for (_k = 0, _len2 = refs.length; _k < _len2; _k++) {
        ref = refs[_k];
        itemRefs[ref] = destIdx;
      }
      _results = [];
      for (_l = 0, _len3 = destRefs.length; _l < _len3; _l++) {
        ref = destRefs[_l];
        _results.push(itemRefs[ref] = idx);
      }
      return _results;
    },
