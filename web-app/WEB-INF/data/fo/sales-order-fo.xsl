...
<xsl:stylesheet version="1.0"
    ...
    <fo:root>
      ...
      <fo:page-sequence master-reference="default" language="de"
        ...
        <fo:flow flow-name="xsl-region-body">
          <xsl:apply-templates select="key('entries', 'salesOrder')"/>
        </fo:flow>
      </fo:page-sequence>
  ...
  <xsl:template match="entry[@key='salesOrder']">
  ...
  <xsl:template match="entry[@key='salesOrder']/billingAddrStreet">
    <fo:block font-size="9pt" space-after="2mm">
      <xsl:value-of select="."/>
    </fo:block>
  </xsl:template>

  <xsl:template match="entry[@key='salesOrder']/billingAddrLocation">
  ...
  <xsl:template match="entry[@key='salesOrder']/items">
  ...
  <xsl:template match="entry[@key='subtotalNet']">
    <fo:table-row border-before-color="#5F6A72" border-before-style="solid"
      ...
      <fo:table-cell padding="0.5mm 1mm">
        <fo:block font-weight="bold">Bestellsumme zzgl. MwSt.</fo:block>
      </fo:table-cell>
  ...
  <xsl:template match="entry[@key='total']">
    <fo:table-row>
      ...
      <fo:table-cell padding="0.5mm 1mm">
        <fo:block font-weight="bold">Bestellsumme einschl. MwSt.</fo:block>
      </fo:table-cell>
      <fo:table-cell><fo:block></fo:block></fo:table-cell>
      <fo:table-cell padding="0.5mm 1mm" text-align="right">
        <fo:block font-weight="bold">
