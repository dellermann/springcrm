...
# This mixin contains various static extensions for jQuery.
#
# @mixin
...
FontSizeWidget =
  ...
  _onChangeFontSize: (event) ->
    ...
    clsCurrent = "#{@widgetBaseClass}-current"
...
RemoteListWidget =
  ...
  _computeUrl: (url) ->
    el = @element
    params = undefined
    url = url or el.data("load-url")
    params = el.data("load-params")
    url = $.param.querystring(url, params)  if params
    url
